...
public class AnalyseIAController {
    ...
    private void lancerAnalyseIA() {
        ...
        new Thread(() -> {
            System.out.println("Démarrage analyse IA pour : " + nomQ);
            ...
            try {
                ...
                System.out.println("❌ Exception dans lancerAnalyseIA : " + ex.getMessage());
            ...
            Platform.runLater(() -> {
                ...
                // Toujours afficher la zone résultat
                boxResultat.setVisible(true);
                boxResultat.setManaged(true);

                if (resultatFinal != null && !resultatFinal.isBlank()) {
                    // ✅ Succès OU message d'erreur lisible : afficher dans le TextArea
                    taAnalyse.setText(resultatFinal);
                    lblErreur.setVisible(false);
                    lblErreur.setManaged(false);
                    System.out.println("✅ Analyse affichée");
                } else {
                    // Cas null/vide (ne devrait plus arriver)
    ...
    private void styliserBouton(Button btn, String couleur, String couleurHover) {
        ...
        btn.setStyle(base);
        btn.setOnMouseEntered(e -> btn.setStyle(base.replace(couleur, couleurHover)));
