...
public class TraitementModificationController implements Initializable {
    ...
    @FXML private TextField txtNomPsychologue;
    ...
    @FXML private Label lblErreurStatut;
    @FXML private Label lblErreurPriorite;
    @FXML private Label lblErreurDateDebut;
    @FXML private Label lblErreurDateFin;
    @FXML private Label lblErreurDosage;
    @FXML private Label lblErreurEtudiant;
    @FXML private Label lblErreurObjectif;
    ...
    public void setTraitement(Traitement traitement) {
        ...
        switch (traitement.getStatut()) {
            case EN_COURS: rbEnCours.setSelected(true); break;
            case TERMINE: rbTermine.setSelected(true); break;
            case SUSPENDU: rbSuspendu.setSelected(true); break;
        }

        // Priorité
        switch (traitement.getPriorite()) {
            case BASSE: rbPrioriteBasse.setSelected(true); break;
            case MOYENNE: rbPrioriteMoyenne.setSelected(true); break;
            case HAUTE: rbPrioriteHaute.setSelected(true); break;
        }
        ...
        // Dosage
        String dosage = traitement.getDosage();
        if (dosage != null && !dosage.isEmpty()) {
            String[] parts = dosage.split(" ");
            if (parts.length >= 2) {
                try {
                    spinnerDosage.getValueFactory().setValue(Integer.parseInt(parts[0]));
                    // Récupérer l'unité (tout après le premier nombre)
                    String unite = dosage.substring(parts[0].length()).trim();
                    cmbUniteDosage.setValue(unite);
    ...
    private void modifierTraitement() throws SQLException {
        if (traitementSelectionne == null) {
            throw new SQLException("Aucun traitement à modifier");
        }

        traitementSelectionne.setTitre(txtTitre.getText().trim());
        traitementSelectionne.setType(cmbType.getValue());
        traitementSelectionne.setCategorie(cmbCategorie.getValue());

        RadioButton selectedStatut = (RadioButton) statutGroup.getSelectedToggle();
        if (selectedStatut == rbEnCours) traitementSelectionne.setStatut(StatutTraitement.EN_COURS);
        else if (selectedStatut == rbTermine) traitementSelectionne.setStatut(StatutTraitement.TERMINE);
        else if (selectedStatut == rbSuspendu) traitementSelectionne.setStatut(StatutTraitement.SUSPENDU);

        RadioButton selectedPriorite = (RadioButton) prioriteGroup.getSelectedToggle();
        if (selectedPriorite == rbPrioriteBasse) traitementSelectionne.setPriorite(PrioriteTraitement.BASSE);
        else if (selectedPriorite == rbPrioriteMoyenne) traitementSelectionne.setPriorite(PrioriteTraitement.MOYENNE);
        else if (selectedPriorite == rbPrioriteHaute) traitementSelectionne.setPriorite(PrioriteTraitement.HAUTE);
        ...
        traitementSelectionne.setDosage(dosage + " " + cmbUniteDosage.getValue());

        traitementSelectionne.setEtudiantId(cmbEtudiant.getValue().getUserId());
